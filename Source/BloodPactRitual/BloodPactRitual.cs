...
namespace Blood_Pact_Ritual.BloodPactRitual
{
    public class BloodPactRitual
    {
        ...
        {                    
            // check if this is really a blood pact attemps
        ...
        private const float PrisonerRecruitChanceFactor = 10f; // much more efficient than normal recruit ways.. but more risky too
        ...
        private static void MakeMoteBubble(Pawn pawn, Texture2D icon, bool good)
        {
            ...
            var moteBubble = (MoteBubble) ThingMaker.MakeThing(good ? ThingDefOf.Mote_ThoughtGood : ThingDefOf.Mote_ThoughtBad);
        ...
        private static void PostPactCreationAttempt(Pawn recipient, Pawn initiator, PawnReaction recipientReaction, PawnReaction initiatorReaction, bool success)
        {
            ...
            if (success)
            {
                ...
                if (Settings.AllowRevolt)
                {
                    ...
                    Find.LetterStack.ReceiveLetter(
                        ...
                        (key + "_Content").Translate(initiator.NameShortColored, recipient.NameShortColored,bestChances.ToStringPercent()),
        ...
        internal static bool CreatePact(Pawn recipient, Pawn initiator, bool addRelation = true)
        {
...
//            initiator.health.AddHediff(instigatorHediff);
        ...
        internal static bool IsValidForKeepingBloodPact(Pawn pawn)
        {
            // only from player faction
            return pawn != null && pawn.relations != null && pawn.Faction != null && pawn.Faction.IsPlayer;
        }

        /// <returns>true if pawn can be the initator in a blood pact ceration.</returns>
        internal static bool IsValidForInitiatingBloodPact(Pawn pawn)
        {
            return IsValidForKeepingBloodPact(pawn) && CanPerformRitual(pawn) && pawn.RaceProps.Humanlike;
        }

        internal static bool IsRecruitablePrisoner(Pawn prisoner)
        {
